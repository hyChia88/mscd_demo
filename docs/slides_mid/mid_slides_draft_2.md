...
> 🎤 **[~60s]:** These the previous observations and motivation defined, I would like to work on interpreting the data flow between the stakeholders and lead to my main research question:
How can AI act as an interpreter layer to reliably align unstructured site evidence with project data in AEC workflows, with minimal information
loss and zero-hallucinations?

> I unpack this into three sub-questions and each  maps each research question directly to a concrete, measurable prototype action — which is the CDR methodology in practice. 
> RQ1 is about cross-modal alignment with Neuro, with task: given a site photo and a short note, can we correctly localise the relevant IFC element or GUID?
RQ2 is about schema-aware mapping with Symbolic: once we know the element and the context, can we map that into valid compliance fields.?
RQ3 is about agentic policy, with Governanc: if we orchestrate these models through an agent correctly and get better time-to-correct compared to a simple, non-agentic pipeline?

> Due to time constraints, and also to give the work a clear, well-defined sandbox in which to experiment, I narrow down the research scope: I would focus on Prefabricated Prefinished Volumetric Construction (PPVC) projects which usually required IFC-centric regulatory workflows in Singapore AEC ecosystem. 
...
- **RQ1 · Neuro** — Top-1 accuracy · SSR · GT-in-pool rate · modality ablation Δ (MA/MB/MC ± 4D)
- **RQ2 · Symbolic** — GT-in-pool @ 100% · fabricated GUID count · fallback rate
- **RQ3 · Governance** — failure detection precision · `fallback_triggered` accuracy
- **All** — latency (V2 0.3 ms pre-computed vs V1 ~15 s/case)
...
> 🎤 **[~40s]:** Three engineering lessons from failure analysis. First: the attribute entropy bottleneck is real — 46 identical windows per floor, CLIP and attribute filters both plateau at 2.2%. The elements are mathematically identical in isolation; only spatial topology distinguishes them. The scene graph creates a topological fingerprint — by requiring a window to be adjacent to a specific column, the query filters out visually identical windows that don't share that configuration. That's the core motivation for V2.3. Second: storey is the highest-leverage single field — one wrong storey eliminates the ground truth from the pool entirely. Third: the symbolic layer is robust — 100% GT-in-pool across 83 hard-negative cases, zero fabricated GUIDs.

1. **Attribute entropy bottleneck** — 46 identical elements/floor; CLIP + attribute filtering plateau at **2.2% Top-1** (AP building)
   - *Only spatial topology distinguishes them → V2.3 topological fingerprint*

2. **Storey = highest-leverage field** — one wrong storey eliminates GT from pool entirely
   - *Over-reduction: 74.3% Prompt vs 64.7% LoRA → storey QA priority for synth_v0.5*

3. **Symbolic layer is hallucination-proof** — H2 hard-negative eval: **100% GT-in-pool, 0 fabricated GUIDs** (83 cases)
   - *Two-step predicate relaxation catches failures without ground-truth labels → RQ3*

> *"The scene graph creates a topological fingerprint — spatial relationships uniquely identify elements that are visually identical."*
...
## 16. Open Questions — Seeking Feedback

> 🎤 **[~Q&A]:** I have a few open questions I'd love feedback on before finalising the thesis scope.

**Q1 — V1 Agent & V2 Scope**
- V1 ReAct agent: high latency (~8 min / 84 cases), non-deterministic → limited as experiment instrument
- **Option A:** Wrap V2 as a retrieval server; V1 agent calls it on query trigger → neuro-symbolic agent
- **Option B:** Conclude agent mode is not suitable for this retrieval task; reframe RQ3 around confidence-based escalation *within* V2
- *Which better serves the thesis scope?*

---

**Q2 — Multimodal Alignment Strategy**
- Current: LoRA fine-tune (image + text + floorplan) → spatial triplets → IFC grounding via scene graph
- Alternative: cross-modal projector (LLaVA / InternVL style) → align visual features directly to IFC schema embeddings
- *Is the spatial triplet intermediate the right abstraction, or would a projector-based approach be more defensible?*

---

**Q3 — Generalisability Boundary**
- Trained & evaluated on 3 IFC buildings
- *What is the minimum generalisability claim the thesis should make — model-specific, building-type-specific, or IFC-schema-general?*

---

**Q4 — Evaluation Sufficiency**
- Current: 50 holdout cases × 6 modality conditions × 2 profiles = 600 traces
- *Is this sufficient evidence for the thesis claims, or is a third condition (retrieval-only, no neuro layer) needed as a baseline?*
