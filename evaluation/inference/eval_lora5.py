"""
MSCD VLM LoRA_5 Evaluation — Modal GPU Inference Script

Runs LoRA_5 constraint extraction on Modal GPU for all evaluation cases.
...
LoRA_5 changes from eval.py (LoRA_3):
  1. Default adapter: /mscd-lora-v5/final (was /mscd-lora/final)
  2. max_new_tokens: 512 (was 256) — multi-triplet JSON is longer
  3. Output path: /mscd-lora-v5/eval_constraints_... (was /mscd-lora/...)
  4. App name: mscd-vlm-lora5-eval (was mscd-vlm-lora-eval)
...
Usage:
    ...
    modal run training/eval_lora5.py --adapter-dir /mscd-lora-v5/final
    modal run training/eval_lora5.py --limit 5  # Quick test

    # v0.5 topology cases (69 cases with spatial relations)
    modal run training/eval_lora5.py --cases /data/v05_test.jsonl --condition-override MC

    # v0.4 holdout (50 cases, backward compatibility check)
    modal run training/eval_lora5.py --condition-override MB
"""
...
# LoRA_5 test file (training conversation format → converted to eval format)
LORA5_TEST_SRC = DATA_ROOT / "datasets" / "synth_v0.5" / "train" / "lora5_test.jsonl"
LORA5_SKELETON_PATHS = [
...
# v0.5 test cases — 75 cases (auto-converted from lora5_test.jsonl)
V05_CASES_FILE = PROJECT_ROOT / "evaluation" / "cases" / "cases_v5_test.jsonl"

# ── Convert lora5_test.jsonl → eval case format ─────────────────────────────

def _convert_lora5_test():
    """Convert lora5_test.jsonl (training format) → cases_v5_test.jsonl (eval format).
    ...
    if V05_CASES_FILE.exists() and \
       V05_CASES_FILE.stat().st_mtime >= LORA5_TEST_SRC.stat().st_mtime:
        return  # Already up-to-date

    print(f"Converting {LORA5_TEST_SRC.name} → {V05_CASES_FILE.name} ...")

    # Load skeletons for ground truth
    skeletons = {}
    for spath in LORA5_SKELETON_PATHS:
        if spath.exists():
...
if LORA5_TEST_SRC.exists():
    _convert_lora5_test()


# ── Modal infrastructure ─────────────────────────────────────────────────────

app = modal.App("mscd-vlm-lora5-eval")
...
eval_image = (
    ...
    # v0.5 BH images + floorplans (NEW in LoRA_5)
    ...
    # v0.5 DXA images + floorplans (floorplans NEW in LoRA_5)
...
def _build_messages(case: dict, system_prompt: str,
                    modality_mode: str = "") -> list:
    """Build ChatML messages for VLM inference.

    modality_mode controls the modality ablation:
        "" or "MC" — all images (site + floorplan)
        "FP"       — floorplan only (drop site photos)
        "SITE"     — site photo only (drop floorplan)
        "MA"       — text only (drop all images)
    """
    user_content = []
    inputs = case.get("inputs", {})
    case_id = case.get("case_id", "")

    # Site photos (skip if FP-only or MA)
    ...
    # Floorplan patch (skip if SITE-only or MA)
...
def _normalize_sr_field(parsed: dict) -> dict:
    ...
    LoRA_5 model sometimes outputs the old LoRA_3 field name 'relations' instead
...
def run_eval(
    adapter_dir: str = "/mscd-lora-v5/final",
    limit: int = 0,
    condition_override: str = "",
    cases_file: str = "",
    modality_mode: str = "",
):
    """Run LoRA_5 constraint extraction on all cases (Modal A100).

    modality_mode: "" (default/MC), "FP" (floorplan only), "SITE" (site only), "MA" (text only)
    """
    import torch
    ...
    if zero_shot:
        ...
        print("=" * 60)
        print("MSCD LoRA_5 Evaluation (Modal A100)")
        print("=" * 60)
    ...
    for idx, case in enumerate(cases, 1):
        ...
        messages = _build_messages(masked_case, system_prompt, modality_mode=modality_mode)
        ...
        try:
            ...
            # LoRA_5: max_new_tokens=512 (was 256 in LoRA_3)
    ...
    if modality_mode:
        tag = f"{tag}_{modality_mode}"
    elif condition_override:
        tag = f"{tag}_{condition_override}"
    output_path = f"/checkpoints/mscd-lora-v5/eval_constraints_{tag}.jsonl"
...
def main(
    ...
    """Launch LoRA_5 evaluation on Modal GPU.

    Args:
        cases: Remote path to cases JSONL inside Modal container.
               Use "/data/v05_test.jsonl" for v0.5 topology cases (92 cases).
               Default: "/data/test_holdout.jsonl" (v0.4 holdout, 50 cases).
        modality: Modality ablation mode — "FP" (floorplan only), "SITE" (site only),
                  "MA" (text only), "" (all images, default).
    """
    cases_label = cases if cases else str(DEFAULT_CASES_FILE)
    print("Launching MSCD LoRA_5 evaluation on Modal...")
    print(f"  Adapter:  {adapter_dir}")
