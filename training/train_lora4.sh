#!/usr/bin/env bash
# ============================================================================
# MSCD VLM LoRA_4 Training Launcher
#
# Validates prerequisites, runs training on Modal GPU, downloads adapter
# with timestamped name to avoid overwrites.
#
# Changes from LoRA_3:
#   - Points to lora4_train.jsonl / lora4_test.jsonl
#   - Volume path: /mscd-lora-v4/final
#   - Adapter output: v4_lora_qwen_<timestamp>
#   - Default 5 epochs (smaller dataset needs more passes)
#
# Usage:
#   ./training/train_lora4.sh                          # Default config (5 epochs)
#   ./training/train_lora4.sh --epochs 5 --lr 2e-4     # Custom config
#   ./training/train_lora4.sh --download-only           # Download adapter only
# ============================================================================
...
# LoRA_4 training data
TRAIN_DIR="$DATA_ROOT/datasets/synth_v0.5/train"
TRAIN_FILE="$TRAIN_DIR/lora4_train.jsonl"
TEST_FILE="$TRAIN_DIR/lora4_test.jsonl"
...
MODAL_VOLUME_PATH="/mscd-lora-v4/final"
RUN_ID="$(date +%Y%m%d_%H%M%S)"
ADAPTER_LOCAL="$PROJECT_DIR/models/adapters/v4_lora_qwen_${RUN_ID}"
...
if [[ "${1:-}" == "--download-only" ]]; then
    info "Downloading trained LoRA_4 adapter from Modal volume..."
...
echo "============================================================"
echo "  MSCD VLM LoRA_4 Training — Pre-flight Checks"
echo "============================================================"
echo ""
...
sr_counts = Counter()
for r in recs:
    asst = next(m for m in r['messages'] if m['role'] == 'assistant')
    label = json.loads(asst['content'])
    sr_counts[len(label.get('spatial_relations', []))] += 1
n_sr = sum(v for k, v in sr_counts.items() if k > 0)
print(f'  SR ratio: {n_sr}/{len(recs)} ({100*n_sr/len(recs):.0f}%)')
print(f'  By SR count: {dict(sorted(sr_counts.items()))}')
print(f'  2-hop records: {sr_counts.get(2, 0)}')
" 2>/dev/null || warn "Dataset audit failed (Python not available)"

echo ""
echo "============================================================"
echo "  Launching LoRA_4 Training on Modal GPU (A100)"
echo "  Run ID: $RUN_ID"
echo "============================================================"
echo ""
...
cd "$PROJECT_DIR"
modal run --detach training/train_lora4.py "$@"
...
echo "  modal app logs mscd-vlm-lora4-train"
echo "  # or WandB: project=mscd-vlm-lora"
echo ""
echo "When complete, download the adapter:"
echo "  ./training/train_lora4.sh --download-only"
