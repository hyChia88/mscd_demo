"""
MSCD VLM LoRA_4 Training — Modal GPU Training Script
...
LoRA_4 changes from LoRA_3:
  - 4 predicates: FILLS, ADJACENT_TO, CONTINUOUS, CONNECTS_TO (was 3)
  - 2-hop multi-triplet records (94 records with [FILLS, CONNECTS_TO])
  - 75% SR ratio (was 44%), 0 noFP+SR (was 171)
  - Storey normalized to floor NUMBER only ("1" not "1 - First Floor")
  - Modality dropout: site photo drop + floorplan drop + 4D metadata mask
  - Dataset: 553 train / 75 test (augmented from 225 KEEP skins + 903 v0.4)

Based on Unsloth's official Qwen2.5-VL vision fine-tuning notebook:
  Qwen2_5_VL_(7B)_Vision.ipynb

Usage:
    modal run training/train_lora4.py
    modal run training/train_lora4.py --epochs 5 --lr 2e-4 --lora-r 16

    # Download trained adapter after training:
    modal volume get mscd-checkpoints /mscd-lora-v4/final ./models/adapters/v4_lora_qwen
...
# synth_v0.5 — LoRA_4 training data (553 train / 75 test)
V05_DIR     = DATASETS_DIR / "synth_v0.5" / "train"
TRAIN_JSONL = V05_DIR / "lora4_train.jsonl"
TEST_JSONL  = V05_DIR / "lora4_test.jsonl"
...
app = modal.App("mscd-vlm-lora4-train")
...
train_image = (
    ...
    # Bake LoRA_4 training JSONL files
    .add_local_file(str(TRAIN_JSONL), remote_path="/data/train/lora4_train.jsonl")
    .add_local_file(str(TEST_JSONL),  remote_path="/data/train/lora4_test.jsonl")
...
class TrainConfig:
    ...
    # Training — 5 epochs default (553 samples, need more passes than LoRA_3's 1377)
    epochs: int = 5
    batch_size: int = 2
    ...
    wandb_run: str = "qwen25vl-7b-r16-lora4-2hop-v05"

    # Paths (inside Modal container)
    train_file: str = "/data/train/lora4_train.jsonl"
    test_file: str = "/data/train/lora4_test.jsonl"
    output_dir: str = "/checkpoints/mscd-lora-v4"
...
def train(
    ...
    # ── 1. Verify data ───────────────────────────────────────────────────
    print("=" * 60)
    print("MSCD VLM LoRA_4 Training (2-hop multi-triplet)")
    print("=" * 60)
    ...
    print("Download with:")
    print(f"  modal volume get mscd-checkpoints /mscd-lora-v4/final "
          f"./models/adapters/v4_lora_qwen")
...
def _run_inference_check(model, tokenizer, test_samples, step=0):
    ...
    LoRA_4 checks: JSON validity, ifc_class, storey_name, spatial_relations
    ...
    if wandb.run is not None:
        try:
            wandb.log({
                ...
                "test_spatial_hop1_acc": spatial_acc,
                "test_spatial_hop2_acc": hop2_acc,
...
def main(
    ...
    """Launch LoRA_4 training on Modal GPU."""
    ...
    print("Launching MSCD VLM LoRA_4 training on Modal...")
    ...
    print(f"\nMonitor on WandB: project=mscd-vlm-lora  run={wandb_run or 'qwen25vl-7b-r16-lora4-2hop-v05'}")
    ...
    print(f"\nDownload adapter:")
    print(f"  modal volume get mscd-checkpoints /mscd-lora-v4/final ./models/adapters/v4_lora_qwen")
